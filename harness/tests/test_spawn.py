...
mode = argv[0]
base = {{"schema_version": "2", "backend": "ggml", "provider": "cpu", "device": "fake",
...
def test_run_argv_shape_and_validation(tmp_path):
    ...
    result = spawn.run(cmd, model_path=Path("/m/model.gguf"), quant="q4", ep="cpu",
    ...
    assert argv[0] == "run"
    assert argv[1:7] == ["--model", "/m/model.gguf", "--quant", "q4", "--ep", "cpu"]
...
def test_run_omits_deadline_when_unset(tmp_path):
    cmd = _fake_backend(tmp_path)
    spawn.run(cmd, model_path=Path("/m/model.gguf"), quant="q4", ep="cpu",
...
def test_sweep_argv_shape_and_runs_unsampled(tmp_path):
    cmd = _fake_backend(tmp_path)
    result = spawn.sweep(cmd, model_path=Path("/m/model.gguf"), quant="q4", ep="vulkan",
                         deadline_ms=600000)
...
def test_backstop_kills_and_reports_timeout(tmp_path):
    ...
    result = spawn.probe([sys.executable, str(hang)], ep="cpu", backstop_s=0.5)
